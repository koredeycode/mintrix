...
export function ProjectPage() {
  ...
  return (
    <div className="h-screen w-screen bg-slate-50 dark:bg-[#0c0919] text-slate-800 dark:text-slate-100 flex flex-col overflow-hidden select-none font-sans">
      {/* Top Navbar */}
      <Navbar>
        <div className="flex items-center gap-3">
          <Link
            to="/"
            className="p-1.5 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white rounded-xl bg-slate-100 dark:bg-[#15102c] border border-slate-200 dark:border-slate-800/80 transition-colors"
            title="Back to Collections"
          >
            <ArrowLeft className="w-3.5 h-3.5" />
          </Link>

          <div className="hidden lg:flex items-center gap-2 border-r border-slate-200 dark:border-slate-800/80 pr-3">
            <span className="font-extrabold text-xs text-slate-900 dark:text-white truncate max-w-[140px]">
              {project.name}
            </span>
            <span className="text-[10px] font-mono text-indigo-600 dark:text-indigo-400 font-bold bg-indigo-50 dark:bg-indigo-950/80 px-1.5 py-0.5 rounded border border-indigo-200 dark:border-indigo-800/80">
          ...
          <form onSubmit={handleStartGeneration} className="flex items-center gap-2">
            <div className="flex items-center bg-slate-100 dark:bg-[#090616] border border-slate-200 dark:border-slate-800/80 rounded-xl p-0.5">
              <span className="px-2 text-[10px] font-mono font-medium text-slate-500 hidden sm:inline">
                Editions:
              </span>
              <input
                type="number"
            ...
            <button
              ...
              className="px-3 py-1.5 sleek-button text-xs font-semibold rounded-xl flex items-center gap-1.5 shadow-xs"
            >
          ...
          {activeJob && activeJob.status === "running" && (
            <div className="hidden sm:flex items-center gap-1.5 text-xs text-indigo-600 dark:text-indigo-400 font-mono font-bold">
              <RefreshCw className="w-3.5 h-3.5 animate-spin" />
              <span>
                {activeJob.currentEdition}/{activeJob.totalEditions} (Active)
              </span>
            </div>
          )}
          ...
          <button
            ...
            className="px-2.5 py-1.5 rounded-xl text-xs font-semibold bg-slate-100 dark:bg-[#15102c] border border-slate-200 dark:border-slate-800/80 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white flex items-center gap-1.5 transition-colors"
          >
