...
import { GeneratorHubModal } from "@/components/studio/GeneratorHub";
...
import { ArrowLeft, Grid, Play, RefreshCw, Settings, SlidersHorizontal, Sparkles } from "lucide-react";
...
export function ProjectPage() {
  ...
  const [isGenHubOpen, setIsGenHubOpen] = useState(false);
  ...
  return (
    <div className="h-screen w-screen bg-slate-50 dark:bg-[#080612] text-slate-800 dark:text-slate-100 flex flex-col overflow-hidden select-none font-sans">
      ...
      <Navbar
        leftControls={
          <div className="flex items-center gap-2.5">
            ...
            <Link
              to="/"
              className="font-heading font-extrabold text-sm tracking-wider uppercase text-slate-900 dark:text-white hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors shrink-0"
            >
              Mintrix
            </Link>
          </div>
        ...
        centerControls={
          <div className="flex items-center gap-2 font-mono text-xs">
            <span
              className="font-heading font-extrabold text-xs text-slate-900 dark:text-slate-200 truncate max-w-[180px]"
              title={project.name}
            >
              {project.name}
            </span>

            <span className="text-[10px] font-mono text-indigo-600 dark:text-indigo-400 font-bold bg-indigo-50 dark:bg-indigo-950/80 px-2 py-0.5 rounded-md border border-indigo-200 dark:border-indigo-800/80 shrink-0">
            ...
            <button
              ...
              className="p-1.5 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white rounded-lg bg-slate-100 dark:bg-[#140f29] border border-slate-200 dark:border-slate-800 transition-colors shrink-0 cursor-pointer"
        ...
        rightControls={
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => setIsGenHubOpen(true)}
              disabled={generating || activeJob?.status === "running"}
              className="px-4 py-1.5 sleek-button text-xs font-bold rounded-xl flex items-center gap-2 shadow-xs cursor-pointer"
            >
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-slate-100 dark:bg-[#15102c] border border-slate-200 dark:border-slate-800/80 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white flex items-center gap-1.5 transition-colors cursor-pointer"
            >
      ...
      {/* Main Studio - Clean 2-Column Layout */}
      ...
      {/* Floating AI Copilot Trigger & Widget Container (Shifted up to avoid overlaying active trait stack) */}
      <button
        type="button"
        onClick={() => setIsCopilotOpen(!isCopilotOpen)}
        className="fixed bottom-20 right-5 z-40 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-full font-bold text-xs shadow-2xl flex items-center gap-2 hover:scale-105 transition-all cursor-pointer border border-indigo-400/30"
        title="Toggle Floatable AI Copilot"
      >
      ...
      {isCopilotOpen && (
        <div className="fixed bottom-32 right-5 z-50 w-80 sm:w-96 h-[480px] bg-white dark:bg-[#130f26] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-4 duration-200">
          <AICopilot
      ...
      {/* Project Configuration Settings Modal */}
      ...
      {/* Advanced Generation Engine & GIF Controls Modal */}
      <GeneratorHubModal
        isOpen={isGenHubOpen}
        projectId={project.id}
        onClose={() => setIsGenHubOpen(false)}
        onStartRun={(jobId) => {
          setIsGenHubOpen(false);
          setIsGalleryOpen(true);
          api.get<GenerationJob>(`/generation/status/${jobId}`).then(setActiveJob).catch(() => {});
          revalidator.revalidate();
        }}
      />
    </div>
