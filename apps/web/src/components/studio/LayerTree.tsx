  ...
  return (
    <div
      ...
      className={`p-2.5 rounded-xl border cursor-pointer transition-all flex items-center gap-3 ${
        ...
        isSelected
          ? "bg-indigo-50/80 dark:bg-indigo-950/60 border-indigo-500 text-slate-900 dark:text-white shadow-xs"
          : "bg-white dark:bg-[#15102c] border-slate-200 dark:border-slate-800/80 hover:border-indigo-500/80"
      }`}
    >
      ...
      <div className="w-11 h-11 rounded-lg bg-slate-100 dark:bg-[#090616] border border-slate-200 dark:border-slate-800 flex items-center justify-center shrink-0 overflow-hidden shadow-inner">
...
export function LayerTree({
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <div className="studio-panel h-full flex flex-col border-r border-slate-200 dark:border-slate-800/80 select-none bg-slate-50/50 dark:bg-[#110d24]">
        {/* Panel Header */}
        <div className="p-3 border-b border-slate-200 dark:border-slate-800/80 flex items-center justify-between bg-white dark:bg-[#15102c] shrink-0">
          <div className="flex items-center gap-1.5 min-w-0">
            <LayersIcon className="w-4 h-4 text-indigo-500 shrink-0" />
            <h3 className="font-bold text-slate-900 dark:text-slate-100 text-xs tracking-tight truncate">
              Layers Navigator
            </h3>
            <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-indigo-50 dark:bg-indigo-950/80 text-indigo-600 dark:text-indigo-400 font-semibold border border-indigo-200 dark:border-indigo-800/80 shrink-0">
              {localLayers.length}
            </span>
          </div>
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="px-2 py-1 rounded text-xs font-semibold bg-indigo-50 dark:bg-indigo-950/80 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900 transition-colors flex items-center gap-0.5 shrink-0"
        ...
        {showAddSection && (
          <div className="p-3 border-b border-slate-200 dark:border-slate-800/80 bg-white dark:bg-[#15102c] space-y-2.5 shrink-0">
            <div>
                    ...
                    <button
                      ...
                      className={`px-2 py-1 rounded-md text-[11px] font-medium border transition-colors ${
                        exists
                          ...
                          : "bg-white dark:bg-[#0c0919] border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:border-indigo-500/80"
                      }`}
                    >
                      + {preset}
                    </button>
            ...
            <form onSubmit={handleAddLayer} className="flex gap-2">
              <input
                ...
                className="flex-1 px-2.5 py-1.5 bg-slate-50 dark:bg-[#0c0919] border border-slate-200 dark:border-slate-800 rounded-lg text-xs text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-indigo-500"
              />
              <button
                type="submit"
                disabled={creating}
